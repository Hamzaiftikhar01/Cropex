      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => (
            <div
              key={feature.title}
              className="group rounded-2xl border border-earth-100 bg-white p-6 shadow-soft transition-all duration-300 hover:border-earth-200 hover:shadow-card"
            >
              <div className="mb-4 inline-flex rounded-xl bg-earth-50 p-3 text-earth-600 ring-1 ring-earth-100 transition-colors group-hover:bg-crop-50 group-hover:text-crop-600 group-hover:ring-crop-100">
                {feature.icon}
              </div>
              <h3 className="text-base font-semibold text-earth-900">{feature.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-earth-500">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
